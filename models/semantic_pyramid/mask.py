...
def bin_mask(chosen: List[int], device, n_mask=7):
    sizes = ((112, 112), (56, 56), (28, 28), (14, 14), (7, 7))
    masks = []

    for i in range(n_mask):
        if i in chosen:
            if i in range(len(sizes)):
                m = torch.ones(sizes[i], device=device)
            else:
                m = torch.ones([1], device=device)
        else:
...
def placeholder_latents(batch, device) -> List[Tensor]:
    sizes = ((64, 112, 112), (128, 56, 56), (256, 28, 28), (512, 14, 14), (512, 7, 7), (4096,), (1000,))
    latents = []
    for s in sizes:
        new_shape = [batch] + [1]*len(s)
        latents.append(torch.zeros(s, device=device).repeat(new_shape))
    return latents


def chosen_mask(chosen, device) -> List[Tensor]:
    sizes = ((112, 112), (56, 56), (28, 28), (14, 14), (7, 7), (4096,), (1000,))
    masks = []
    # chosen = [5, 6]
    for s in sizes:
        masks.append(torch.zeros(s, device=device))
    for c in chosen:
        masks[c] = torch.ones(sizes[c], device=device)
    return masks


def chosen_layers(depth: int, n_layers:int = 7) -> List[int]:
    return [i for i in range(depth, n_layers)]
