...
class RealNVP(nn.Module):
    ...
    def log_prob(self, x, y=None):
        u, sum_log_abs_det_jacobians = self.forward(x, y)
        sum_log_abs_det_jacobians = torch.sum(sum_log_abs_det_jacobians, dim=1)
        log_prob = self.base_dist.log_prob(u)
        log_prob = torch.sum(log_prob, dim=1)

        self.mean_log_det_J += float(torch.mean(sum_log_abs_det_jacobians))
        self.count += 1
        self.mean_log_prob -= float(torch.mean(log_prob))
        ...
        return log_prob + sum_log_abs_det_jacobians
