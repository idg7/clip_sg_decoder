...
class WPlusFlowCoach(object):
    ...
    def __log_images(self, title: str, x: Optional[Tensor] = None, y: Optional[Tensor] = None, y_hat: Optional[Tensor] = None):
        with torch.no_grad():
            if y_hat is None:
                if y is None:
    ...
    def mapping_test_iter(self, x: Optional[Tensor]):
        ...
        if self.args.clip_on_orig:
            x_embeddings = self.encoder.encode_image(self.resize(x))
        else:
            y, y_latents = self.autoencoder(self.face_pool(x), return_latents=True)
        ...
        if self.global_step % self.image_log_freq == 0:
            self.__log_images('test', x, self.autoencoder(x))#, x, y_hat)
    ...
    def mapping_train_iter(self, x: Tensor):
        self.grad_manager.requires_grad(self.autoencoder, False)
        self.autoencoder.train(False)
        self.grad_manager.requires_grad(self.encoder, False)
        self.encoder.train(False)
        ...
        if self.args.clip_on_orig:
            y_embeddings = self.encoder.encode_image(self.resize(x))
        else:
            y_embeddings = self.encoder.encode_image(self.resize(y))
        y_embeddings = y_embeddings.float()
        y_embeddings = self.embedding_norm(y_embeddings)
            ...
            for i in pbar:
                ...
                if not train:
                    if self.args.test_on_dataset:
                        loss, iter_loss_dict = self.mapping_test_iter(images)
                    else:
                        loss, iter_loss_dict = self.mapping_random_test_iter()
                else:
                    loss, iter_loss_dict = self.mapping_train_iter(images)
